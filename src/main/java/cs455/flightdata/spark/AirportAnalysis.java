...
public class AirportAnalysis {
    ...
    private static final int UPPER_COUNT_LIMIT = 92625;

    private static final int LOWER_COUNT_LIMIT = 290;

    private static final double PERCENT_TOP_50_LIMIT = 18.06;

    private static final double PERCENT_BOTTOM_50_LIMIT = 8.41;
    ...
    public void analyze(String inputDir, String outputDir, JavaSparkContext context) {
        ...
        JavaRDD<String> airportMap =
                onlyDelayed.flatMap((FlatMapFunction<String, String>) s -> Arrays.asList(s.split(",")[16]));

        JavaPairRDD<String, Integer> frequency =
                airportMap.mapToPair((PairFunction<String, String, Integer>) s -> new Tuple2<>(s,
                        1));
        ...
        JavaPairRDD<String, Integer> filteredUpperCounts =
                counts.filter((Tuple2<String, Integer> upperCountPair) -> upperCountPair._2()
                        >= UPPER_COUNT_LIMIT);

        JavaPairRDD<String, Integer> filteredLowerCounts =
                counts.filter((Tuple2<String, Integer> lowerCountPair) -> lowerCountPair._2()
                        <= LOWER_COUNT_LIMIT);

        SparkUtils.saveCoalescedRDDToJsonFile(filteredUpperCounts,
                outputDir + "/airport-upper-count");
        SparkUtils.saveCoalescedRDDToJsonFile(filteredLowerCounts,
                outputDir + "/airport-lower-count");
        ...
        JavaPairRDD<String, Tuple2<Double, Double>> percentRDD =
                allFlights.mapToPair((String s) -> {
                    ...
                    return new Tuple2<>(originAirport, new Tuple2<>(delayedFlightCount,
                            totalFlightCount));
                })
        ...
        JavaPairRDD<String, Double> filteredUpperPercentDelay =
                percentAirportDelayRDD.filter((Tuple2<String, Double> topPairs) -> topPairs._2()
                        >= PERCENT_TOP_50_LIMIT);

        JavaPairRDD<String, Double> filteredLowerPercentageRDD =
                percentAirportDelayRDD.filter((Tuple2<String, Double> bottomPairs) ->
                        bottomPairs._2() <= PERCENT_BOTTOM_50_LIMIT);

        SparkUtils.saveCoalescedRDDToJsonFile(filteredUpperPercentDelay,
                outputDir + "/airport-upper-percent");
        SparkUtils.saveCoalescedRDDToJsonFile(filteredLowerPercentageRDD,
                outputDir + "/airport-lower-percent");

        JavaPairRDD<Double, String> swappedRDD = percentAirportDelayRDD.mapToPair(Tuple2::swap);
