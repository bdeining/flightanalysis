...
public class DelayByModel {
    ...
    private static double percentTop50Limit = 51.72;
    private static double averageTop50Limit = 105.00;
    private static int countTop50Limit = 5719;
    ...
    public void analyze(String inputDir, String outputDir, JavaSparkContext context) {
        ...
        JavaRDD<String> onlyDelayed = lines.filter(
                (Function<String, Boolean>) s -> {
                    try {
                        if (Integer.parseInt(s.split(",")[15]) > 15 && !s.split(",")[10].equals("NA")) {
                            return true;
                        } else {
                            return false;
                        }
        ...
        JavaRDD<String> modelMap = onlyDelayed.flatMap(
                (FlatMapFunction<String, String>) s -> Arrays.asList(s.split(",")[10]));

        JavaPairRDD<String, Integer> frequency = modelMap.mapToPair(
        ...
        JavaPairRDD<String, Integer> upperCounts =
                counts.filter((Tuple2<String, Integer> countPair) -> countPair._2() >= countTop50Limit);
        ...
        JavaPairRDD<String, Double> filteredUpperAverage =
                answerRDD.filter((Tuple2<String, Double> averagePair) -> averagePair._2() >= averageTop50Limit);

        SparkUtils.saveCoalescedRDDToJsonFile(filteredUpperAverage, outputDir + "/model-upper-average");
        ...
        JavaPairRDD<String, Tuple2<Double, Double>> percentRDD =
                allFlights.mapToPair((String s) -> {
                    ...
                    return new Tuple2<>(airplane, new Tuple2<>(delayedFlightCount, totalFlightCount));
        ...
        JavaPairRDD<String, Double> filteredUpperModelDelayPercentage =
                percentModelDelayRDD.filter((Tuple2<String, Double> upperPercentage) -> upperPercentage._2() >= percentTop50Limit);

        SparkUtils.saveCoalescedRDDToJsonFile(filteredUpperModelDelayPercentage, outputDir + "/model-upper-percent");
        ...
        for (int i = averageList.size() - 1; i > averageList.size() - 50; i--) {
            System.out.println(averageList.get(i)._1() + ":" + averageList.get(i)._2());
        }
