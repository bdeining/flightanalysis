...
public class NumberOfAirlinesDelay implements Serializable {
    ...
    public static void main(String[] args) {
        if (args.length != 4) {
            System.err.println("Usage: FlightCancellation <input-directory> <output-directory> <tail-num-master_file> " +
    ...
    public JavaPairRDD<String, String> createTailNumMappingRdd(JavaSparkContext ctx, String tailNumMasterFile,
                                                                               String manufacturerModelLookupFile) {
        ...
        JavaPairRDD<String, String> tailNumMfrModelCodeRdd = tailNumRdd.mapToPair(
                (PairFunction<String, String, String>) fullLine -> {
        ...
        JavaPairRDD<String, String> codeToModelStringsRdd = mfrModelLookupRdd.mapToPair(
                (PairFunction<String, String, String>) fullLine -> {
                    String mfrModelCode = fullLine.substring(0, 6);
//                    String mfrString = fullLine.substring(8, 37).trim();
                    String modelString = fullLine.substring(39, 58).trim();
        ...
        JavaPairRDD<String, String> tailNumToMfr = codeToMfrModelTailRdd.mapToPair(
                (PairFunction<Tuple2<String, Tuple2<String, String>>, String, String>) valTup ->
                        new Tuple2<>(valTup._2()._2(), valTup._2()._1())
        );
    ...
    public boolean executeAnalysis(JavaSparkContext ctx, JavaRDD<FlightInfo> allFlightsInfo, String outputDir,
                                   JavaPairRDD<String, String> tailNumToModelRdd) {
        ...
        JavaPairRDD<Tuple2<String, String>, Long> numberFlightsEachMonth = airlinesByMonth.mapToPair(
...
//                outputDir + File.separator + "number_airlines_each_month-year");
        ...
        JavaRDD<FlightInfo> occurancesOfDelays = allFlightsInfo.filter(
                (Function<FlightInfo, Boolean>) fci ->
                        !fci.getFlightDelay().equals("NA") && Long.parseLong(fci.getFlightDelay()) > 15
        );
...
//                numberFlightsEachMonth.union(totalsForEachMonth).union(yearMonthDelaySum).groupByKey();
        ...
        JavaPairRDD<Tuple2<String, String>, Iterable<Long>> allFlightsNumAirlinesDelaysEachMonth =
                ctx.union(numberFlightsEachMonth, totalsForEachMonth, yearMonthDelaySum).groupByKey();
        ...
        JavaPairRDD<String, Long> tailNumDelayOccurances = occurancesOfDelays.mapToPair(
        ...
        // count them up
        JavaPairRDD<String, Long> tailNumDelayCounts = tailNumDelayOccurances.reduceByKey(
                (l1, l2) -> l1 + l2
        );
        ...
        JavaPairRDD<String, Tuple2<Long, String>> tailNumDelaysAndModel = tailNumDelayCounts.join(tailNumToModelRdd);

        // swap tail num for model
        JavaPairRDD<String, Long> modelToDelays = tailNumDelaysAndModel.mapToPair(
                (PairFunction<Tuple2<String, Tuple2<Long, String>>, String, Long>) tailNumTup ->
                        new Tuple2<>(tailNumTup._2()._2(), tailNumTup._2()._1())
        ).reduceByKey(
                (l1, l2) -> l1 + l2
        );
